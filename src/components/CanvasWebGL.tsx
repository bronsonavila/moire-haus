...
const CanvasWebGL = ({ cellSize, colorShift, columns, index, rows }: CanvasWebGLProps) => {
  ...
  // Note: page.tsx falls back to CPU renderer for high frequencies (>100k)
  // so we don't need to cap here - WebGL is only used for safe frequency ranges
  const frequency = multiplier

  // WebGL can handle full resolution without downscaling
  ...
  useEffect(() => {
    ...
    // Upload palette textures in order (Cool to Warm): TEAL_NAVY, MINT_INDIGO, LIME_PURPLE, ROSE_VIOLET, PEACH_CHARCOAL, GOLD_NOIR, AMBER
  ...
  useEffect(() => {
    ...
    gl.useProgram(program)
    gl.viewport(0, 0, drawColumns, drawRows)

    // Set uniforms
    gl.uniform2f(uniforms.resolution, drawColumns, drawRows)
    ...
    setCanvasReady(true)
  }, [drawColumns, drawRows, frequency, offset, colorShift, cellSize, index, setCanvasReady])
...
export default memo(CanvasWebGL)
